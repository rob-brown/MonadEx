defmodule Monoid.Law do
  ...
  def closure?(m1, m2) when is_map(m1) and is_map(m2) do
    ...
    is_map(m3) and Map.get(m1, :__struct__) == Map.get(m2, :__struct__) and
      Map.get(m2, :__struct__) == Map.get(m3, :__struct__)
  end
  ...
  def associativity?(monoid1, monoid2, monoid3) do
    Monoid.mappend(Monoid.mappend(monoid1, monoid2), monoid3) ==
      Monoid.mappend(monoid1, Monoid.mappend(monoid2, monoid3))
  end
end
